...
export function PrivateNavigation() {
  return <nav className="private-navigation" aria-label="Navegación de cuenta">
    ...
    <NavLink className={({ isActive }) => `private-nav-link${isActive ? ' is-active' : ''}`} to="/calculator"><span aria-hidden="true">02</span>Calculadora</NavLink><NavLink className={({ isActive }) => `private-nav-link${isActive ? ' is-active' : ''}`} to="/history"><span aria-hidden="true">03</span>Historial</NavLink>
    <NavLink className={({ isActive }) => `private-nav-link${isActive ? ' is-active' : ''}`} to="/account"><span aria-hidden="true">04</span>Cuenta</NavLink></div>
